...
void
rotate_position (char *prow, char *pcol)
{
  *prow = INV_INT (*prow);
  *pcol = INV_INT (*pcol);
}
...
void
edit_nag_toggle_item (struct menu_input_s *m)
{
...
void
edit_nag_save (struct menu_input_s *m)
{
  pushkey = -1;
}

void
edit_nag_help (struct menu_input_s *m)
{
...
int
do_game_write (char *filename, const char *mode, int start, int end)
{
  int i;
  struct userdata_s *d;
  PGN_FILE *pgn;
...
void
do_save_game_overwrite_confirm (WIN * win)
{
  const char *mode = "w";
 ...
 */
void
save_pgn (char *filename, int saveindex)
{
...
piece_can_attack (GAME g, int rank, int file)
{
  ...
  cpi = d->sp.icon
    ...
    pgn_piece_to_int (d->b[RANKTOBOARD (d->sp.srow)]
		      [FILETOBOARD (d->sp.scol)].icon) :
    pgn_piece_to_int (d->b[RANKTOBOARD (d->c_row)]
		      [FILETOBOARD (d->c_col)].icon);
...
void
print_piece (WINDOW * w, int l, int c, char p)
{
  int i, y, ff = 0;
...
void
board_prev_move_history (GAME g)
{
  struct userdata_s *d = g->data;
	  ...
	  if ((row % rowr))
	      ...
	      if ((col % colr))
		{
		  ...
		  if (config.details &&
		      ((!BIG_BOARD
			&& d->
			b[RANKTOBOARD (brow)][FILETOBOARD (bcol)].enpassant)
		       || (BIG_BOARD && d->b[brow][bcol].enpassant)))
		    {
		      p = pi = 'x';
		      attrs = mix_cp (CP_BOARD_ENPASSANT,
				      (attrwhich ==
				       WHITE) ? CP_BOARD_WHITE :
				      CP_BOARD_BLACK,
		  ...
		  if (config.validmoves &&
		      ((!BIG_BOARD
			&& d->b[RANKTOBOARD (brow)][FILETOBOARD (bcol)].valid)
		  ...
		  if (can_attack)
		    {
		      ...
		      chtype a = n && !valid
			? CP_BOARD_PREVMOVE : attrwhich ==
			WHITE ? valid ? CP_BOARD_MOVES_WHITE : CP_BOARD_WHITE
			: valid ? CP_BOARD_MOVES_BLACK : CP_BOARD_BLACK;
		      attrs =
...
void
invalid_move (int n, int e, const char *m)
{
  if (curses_initialized)
    cmessage (ERROR_STR, ANY_KEY_STR, "%s \"%s\" (round #%i)",
	      (e ==
	       E_PGN_AMBIGUOUS) ? _("Ambiguous move") : _("Invalid move"), m,
	      n);
  else
...
void
do_promotion_piece_finalize (WIN * win)
{
  char *p, *str = win->data;
...
void
append_enginebuf (GAME g, char *line)
{
  int i = 0;
  struct userdata_s *d = g->data;
 ...
 */
void
update_status_notify (GAME g, const char *fmt, ...)
{
  va_list ap;
...
int
rav_next_prev (GAME g, BOARD b, int n)
{
  // Next RAV.
  if (n)
    {
...
void
do_clock_input_finalize (WIN * win)
{
  struct userdata_s *d = gp->data;
...
void
do_engine_command_finalize (WIN * win)
{
  struct userdata_s *d = gp->data;
  struct input_data_s *in = win->data;
  int x;
...
void
do_edit_insert_finalize (WIN * win)
{
...
void
really_do_annotate_finalize (struct input_data_s *in, struct userdata_s *d)
{
  HISTORY *h = in->data;
  int len;
...
void
do_find_move_exp_finalize (int init, int which)
{
  int n;
  struct userdata_s *d = gp->data;
  static int firstrun;
...
void
free_history_menu_data (struct history_menu_s **h)
{
  int i;
...
void
get_history_data (HISTORY ** hp, struct history_menu_s ***menu, int m,
		  int turn)
{
  int i, n = 0;
...
void
history_draw_update (struct menu_input_s *m)
{
  GAME g = m->data;
  struct userdata_s *d = g->data;
...
// FIXME RAV
void
history_menu_next (struct menu_input_s *m)
{
  GAME g = m->data;
  struct userdata_s *d = g->data;
  struct history_menu_s **hm = d->data;
...
// FIXME RAV
void
history_menu_prev (struct menu_input_s *m)
{
  GAME g = m->data;
  struct userdata_s *d = g->data;
  struct history_menu_s **hm = d->data;
...
void
history_menu_help (struct menu_input_s *m)
{
...
void
history_menu_view_annotation (struct menu_input_s *m)
{
  GAME g = m->data;
...
void
history_menu_annotate_finalize (WIN * win)
{
  struct input_data_s *in = win->data;
  GAME g = in->moredata;
  struct userdata_s *d = g->data;
...
void
history_menu_annotate (struct menu_input_s *m)
{
  GAME g = m->data;
  char buf[COLS - 4];
  struct input_data_s *in;
...
void
history_menu_details (struct menu_input_s *m)
{
  do_board_details ();
}
 ...
 */
  ...
  mvwaddch (win->w, m->print_line, 1,
	    *p == 'W' ? *p | mix_cp (CP_BOARD_WHITE, CP_HISTORY_WINDOW,
				     ...
				     A_FG_B_BG) : *p | mix_cp (CP_BOARD_BLACK,
							       CP_HISTORY_WINDOW,
							       ATTRS
							       (CP_BOARD_BLACK),
							       A_FG_B_BG));
...
void
do_history_mode_finalize (struct userdata_s *d)
{
  pushkey = 0;
  d->mode = MODE_PLAY;
}
...
void
do_history_mode_confirm (WIN * win)
{
  struct userdata_s *d = gp->data;
  wchar_t str[] = { win->c, 0 };
...
void
do_new_game_from_scratch (WIN * win)
{
  wchar_t str[] = { win->c, 0 };
...
void
do_game_delete_confirm (WIN * win)
{
  int *n;
  wchar_t str[] = { win->c, 0 };
...
void
do_find_game_exp_finalize (int which)
{
  struct userdata_s *d = gp->data;
  int n;
...
void
do_game_save_multi_confirm (WIN * win)
{
  int i;
  wchar_t str[] = { win->c, 0 };
...
void
global_game_next_prev (int which)
{
  struct userdata_s *d;
...
void
loading_progress (long total, long offset)
{

...
notice_style(struct notice_s *n)
{
    ...
    if (n->label)
    {
        ...
        /*
         * Bold only for menu-style notices: bright white (not dim grey).
         * Error dialogs stay normal-intensity red (bold would be bright red).
         */
        vk_widget_set_attrs(VK_WIDGET(n->label),
                            n->is_error ? A_NORMAL : A_BOLD);
    ...
    if (bar)
    {
        ...
        vk_widget_fill(VK_WIDGET(bar), ' ' | COLOR_PAIR(pair));
        vk_box_update(bar);
    }

 ...
 */
char *
pgn_game_to_fen(GAME g, BOARD b)
{
    char tmp[16];
    int row, col;
    int i;
    ...
    *p++ = ' ';
    ...
    if (TEST_FLAG(g->flags, GF_WK_CASTLE) && pgn_piece_to_int(b[7][7].icon) == ROOK && isupper(b[7][7].icon) && pgn_piece_to_int(b[7][4].icon) == KING && isupper(b[7][4].icon))
    {
        *p++ = 'K';
        castle = 1;
    }

    if (TEST_FLAG(g->flags, GF_WQ_CASTLE) && pgn_piece_to_int(b[7][0].icon) == ROOK && isupper(b[7][0].icon) && pgn_piece_to_int(b[7][4].icon) == KING && isupper(b[7][4].icon))
    {
        *p++ = 'Q';
        castle = 1;
    }

    if (TEST_FLAG(g->flags, GF_BK_CASTLE) && pgn_piece_to_int(b[0][7].icon) == ROOK && islower(b[0][7].icon) && pgn_piece_to_int(b[0][4].icon) == KING && islower(b[0][4].icon))
    {
        *p++ = 'k';
        castle = 1;
    }

    if (TEST_FLAG(g->flags, GF_BQ_CASTLE) && pgn_piece_to_int(b[0][0].icon) == ROOK && islower(b[0][0].icon) && pgn_piece_to_int(b[0][4].icon) == KING && islower(b[0][4].icon))
    {
        *p++ = 'q';
        castle = 1;
    }
 ...
 */
HISTORY *
pgn_history_by_n(HISTORY **h, int n)
{
 ...
 */
pgn_error_t
pgn_history_add(GAME g, BOARD b, const char *m)
{
 ...
 */
pgn_error_t
pgn_board_update(GAME g, BOARD b, int n)
{
    BOARD tb;
    int ret = E_PGN_OK;
 ...
 */
pgn_error_t
pgn_int_to_piece(char turn, int n)
{
    int p = 0;
 ...
 */
pgn_error_t
pgn_tag_find(TAG **t, const char *name)
{
    int i;
...
static pgn_error_t
remove_tag(TAG ***array, const char *tag)
{
    TAG **tags = *array;
 ...
 */
pgn_error_t
pgn_tag_add(TAG ***dst, char *name, char *value)
{
    int i;
    TAG **tdata = *dst;
    TAG **a = NULL;
...
static char *
remove_tag_escapes(const char *str)
{
    int i, n;
    int len = strlen(str);
 ...
 */
static int
annotation_text(GAME g, FILE *fp, int terminator)
{
    int c, lastchar = 0;
    int len = 0;
...
static int
rav_text(GAME g, FILE *fp, int which, BOARD o)
{
    struct game_s tg;
    RAV *r;
    ...
    if (which == '(')
    {
       ...
       */
        ...
        if ((g->rav[g->ravlevel].fen = pgn_game_to_fen(g, pgn_board)) == NULL)
        {
            warn("strdup()");
            return 1;
        }
        ...
        if ((g->hp[g->hindex - 1]->rav =
                 calloc(1, sizeof(HISTORY *))) == NULL)
        {
            warn("calloc()");
            return 1;
        }
 ...
 */
static int
parse_fen_line(BOARD b, unsigned *flags, char *turn, char *ply, char *str)
{
    char *tmp;
 ...
 */
pgn_error_t
pgn_board_init_fen(GAME g, BOARD b, char *fen)
{
    int n = -1, i = -1;
    BOARD tmpboard;
    unsigned flags = 0;
   ...
   */
    if ((n >= 0 && i >= 0 && atoi(g->tag[n]->value) == 1) || (i >= 0 && n == -1) || fen)
    {
       ...
       */
        ...
        if (c == '[')
        {
            ...
            if (!tag_section)
            {
                ...
                if (gtotal && pgn_history_total(game[gindex]->hp))
                    game[gindex]->hindex =
                        pgn_history_total(game[gindex]->hp) - 1;
        ...
        if ((isdigit(c) && (nextchar == '-' || nextchar == '/')) || c == '*')
        {
            ...
            if (!game[gindex]->done_fen_tag)
            {
                ...
                game[gindex]->pgn_fen_tag =
                    pgn_tag_find(game[gindex]->tag, "FEN");
        ...
        // PGN: Move text.
        if ((isdigit(c) && c != '0') || VALIDCOL(c) || c == 'N' || c == 'K' || c == 'Q' || c == 'B' || c == 'R' || c == 'P' || c == 'O')
        {
            Ungetc(c, fp);
 ...
 */
static char *
pgn_tag_add_escapes(const char *str)
{
    int i, n;
    int len = strlen(str);
    char buf[MAX_PGN_LINE_LEN] = {0};
...
static void
putstring(FILE *fp, char *str, int *len)
{
    char *p;
 ...
 */
static void
write_comments_and_nag(FILE *fp, HISTORY *h, int *len)
{
    int i;
    char tmp[16];
...
static void
write_move_text(FILE *fp, HISTORY *h, int *len)
{
    Fputc(' ', fp, len);
...
static void
write_all_move_text(FILE *fp, HISTORY **h, int m, int *len)
{
    int i;
    HISTORY **hp = NULL;
    char tmp[16];
...
static char *
compression_cmd(const char *filename, int expand)
{
             ...
             filename[len] == '\0')
    {
        ...
        return strdup(command);
              ...
              filename[len] == '\0') ||
             (filename[len - 3] == '.' &&
              filename[len - 2] == 'b' && filename[len - 1] == 'z' && filename[len] == '\0'))
    {
        if (expand)
...
static int
copy_file(FILE *fp, const char *dst)
{
    FILE *ofp;
    char line[LINE_MAX];
 ...
 */
pgn_error_t
pgn_open(const char *filename, const char *mode, PGN_FILE **result)
{
...
#else
                if ((fd =
                         open(tmp, O_RDWR | O_EXCL | O_CREAT | O_TRUNC,
                              0600)) == -1)
                    goto fail;
#endif
